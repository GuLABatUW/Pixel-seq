...
perl shapeImageSegVnet.pl <parameters>
    ...
    -region|r 	  <str>	  Xlower,Xupper,ylower,yupper (24000, 42000, 6000, 7200)
    -algorithm|a  <str>	  fastGreedy or edgeBetweenness (fastGreedy)
    -final|f      <str>	  the raw final file
    -cut|c        <num>   cutoff of minimum barcodes per group (25)
    -work|w   <str>   work direction
...
if ($featurefile eq "" or $region eq "" or $search eq "" or $lowercut eq "" or $finalF eq "" or $work_dir eq "") {
    print $usage,"\n";
    exit;
}
...
my $gridR = 7000; #the subregion row size
my $gridC = 7000; #the subregion col size
my $overlap = 700; #the overlap region size
my %cellSeg;
my %cellBcount;
my %cell2blist;
my %featureUMI;
...
my $coordName = $work_dir.'spt_U_Mask'.'Coord.txt';
...
my $cgename = $work_dir.'spt_U_Mask'.'CellGeneExp.txt';
...
sub vnetSeg{
	...
	foreach my $key(sort keys %featureUMI){
		open(TT,'>',$work_dir.'regionUSelect.out') || die;
		my %uniqueIdList;
		...
		if($edgeCount>3000){
#			print("Rscript --vanilla ".$work_dir."vNetseq2.R $key regionUSelect.out $search ".$work_dir."\n");
			system("Rscript --vanilla ".$work_dir."vNetseq2.R $key regionUSelect.out $search ".$work_dir);
			maskMerge($work_dir.'cellAsign2.csv',$mergeCount);
			#my $datestring = localtime();
			print '> seg:     ',$mergeCount,"\n";
			$mergeCount++;
		} else {
			print 'skip',"\n";
		}
...
#				$cell2blist{$dat[4].'|'.$dat[5]} = (12000-$dat[2]).'|'.$dat[1];
				$cell2blist{$dat[4].'|'.$dat[5]} = (12000-$dat[3]).'|'.$dat[2];
			} else {
#				$cell2blist{$dat[4].'|'.$dat[5]} = $cell2blist{$dat[4].'|'.$dat[5]}."\t".(12000-$dat[2]).'|'.$dat[1];
				$cell2blist{$dat[4].'|'.$dat[5]} = $cell2blist{$dat[4].'|'.$dat[5]}."\t".(12000-$dat[3]).'|'.$dat[2];
			}
...
sub outputSeg{
	...
	my %cellID;
	my $counter = 1;
	open(IDA, ">", $work_dir."idAssignBarcode.txt") || die;
